def dienas_mekletajs (sis_gads, sis_menesis, si_diena, dz_gads, dz_datums, dz_menesis,   ):
    menesu_dienu_skaits = [0, 31, 28, 31, 30, 31, 30, 31, 31, 31, 30, 30, 30]
...
pagajusie_gadi += garie_gadi
 
#cik pilni meneši ir pagājuši kops pedējās dzimšanas dienas?
...
while menesis != sis_menesis:
    ...
    for i in range(dz_menesis, sis_menesis):
     ...
     dienas_menesos += menesu_dienu_skaits[i]

pagajusas_dienas += dienas_menesos

dienas_atlikums = pagajusas_dienas % 7

if dz_diena <=0:

dz_d = int(input("Lūdzu ievadiet savu dzimšanas gadu!:"))
